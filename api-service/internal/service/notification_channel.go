...
func (s *notificationChannelService) CreateEmailChannel(
	...
	channel := &model.NotificationChannelConfig{
		Code:          req.Code,
		Name:          req.Name,
		Description:   toStringPtr(req.Description),
...
func (s *notificationChannelService) TestEmailChannel(
	...
	if err := s.sendTestEmail(ctx, emailConfig, req.Recipient, testSubject, testContent); err != nil {
...
func (s *notificationChannelService) TestWebhookChannel(
	...
	if channel.ChannelConfig != nil {
		...
		if err := json.Unmarshal(configBytes, &webhookConfig); err != nil {
			s.logger.ErrorContext(ctx, "Failed to unmarshal webhook config", logger.ErrorField(err))
			return &response.TestChannelResponse{
				Success: false,
				Message: s.i18n.T(ctx, "notification.channel.config_parse_failed"),
				Details: err.Error(),
...
func (s *notificationChannelService) sendTestEmail(
	...
	case "tls", "ssl":
		err = s.sendEmailWithTLS(ctxWithTimeout, addr, auth, config.SenderEmail, []string{to}, msg, config.SMTPHost)
	case "none", "":
...
func (s *notificationChannelService) sendTestWebhook(
	...
	// Read response body
	responseBody := make([]byte, 1024) // Limit response size for testing
	n, _ := resp.Body.Read(responseBody)

	return resp.StatusCode, string(responseBody[:n]), nil
}
