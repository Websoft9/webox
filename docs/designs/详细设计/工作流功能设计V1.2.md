...
- [工作流功能详细设计 V1.2](#工作流功能详细设计-v12)
  ...
  - [5. 实现要点与关键数据流](#5-实现要点与关键数据流)
    ...
    - [5.2 Temporal 架构集成](#52-temporal-架构集成)
      - [5.2.1 整体架构](#521-整体架构)
      - [5.2.2 服务端集成](#522-服务端集成)
      - [5.2.3 客户端开发](#523-客户端开发)
    - [5.3 分布式调度实现](#53-分布式调度实现)
      - [5.3.1 多节点任务分配](#531-多节点任务分配)
      - [5.3.2 节点间数据同步](#532-节点间数据同步)
      - [5.3.3 故障转移和重试](#533-故障转移和重试)
    - [5.4 组件实现机制](#54-组件实现机制)
      - [5.4.1 组件执行流程](#541-组件执行流程)
      - [5.4.2 条件分支处理](#542-条件分支处理)
      - [5.4.3 组件注册](#543-组件注册)
      - [5.4.4 参数解析引擎](#544-参数解析引擎)
      - [5.4.5 预置组件列表](#545-预置组件列表)
  - [6. 数据字典设计](#6-数据字典设计)
    - [6.1 系统表](#61-系统表)
    - [6.2 独立表](#62-独立表)
...
3. **Agent Layer**：
   - 部署在每个纳管服务器上
   - 每个 Agent 包含完整的 Workflows（Worker、Activity）、Monitor、Docker Manager 模块
...
1. **前端层（User）**：文本编辑器实现工作流定义（JSON）
2. **API 服务层（API Service）**：工作流管理、任务调度、执行监控
3. **数据存储层（Database）**：MySQL（持久化存储）+ Redis（缓存和状态管理）
4. **Temporal CLI 层**：命令行工具，用于与Temporal Server交互
5. **Temporal Server 层**：工作流引擎核心服务，管理Task Queue
6. **Agent 层**：Websoft9 Agent 作为 Worker 节点，分布式执行任务
   - Worker：执行工作流任务
   - Activity：具体组件执行（Controller、Shell、Http、Email、File operation等）
   - Monitor：监控服务
   - Docker Manager：容器管理
...
```
User → API Service → Database (MySQL/Redis)
                  ↓
            Temporal CLI → Temporal Server → Task Queue
                                          ↓
                                      Agent (Worker)
                                          ↓
                                      Activity执行
...
1. **Workflows 模块**：
   ...
   - **Activity 模块**（组件执行层）：
      - **Controller**：流程控制组件（START、END、CONDITION等）
      - **Shell**：执行Shell命令
      - **Http**：HTTP请求调用
      - **Email**：邮件发送
...
2. **任务执行阶段**：
   ...
   - 根据任务类型调用对应的 Activity 组件
   - Activity 执行具体业务逻辑（如执行Shell命令、发送HTTP请求等）
...
1. 工作流执行时，根据工作流配置（Activity config）确定目标节点
...
| 组件名称  | 功能描述       | 执行位置 | 对应Activity |
| --------- | -------------- | -------- | ------------ |
| START     | 工作流开始节点 | Server   | Controller   |
| END       | 工作流结束节点 | Server   | Controller   |
| CONDITION | 条件判断分支   | Server   | Controller   |
...
| DEPLOY_APP    | 部署应用                  | Worker   | Docker Manager |
...
- **Controller Activity**：处理流程控制逻辑（START、END、CONDITION）
- **Shell Activity**：执行Shell命令
- **Http Activity**：处理HTTP请求
- **Email Activity**：发送邮件和短信
- **File operation Activity**：处理所有文件相关操作（上传、下载、复制、删除、S3操作等）
- **Docker Manager**：通过Docker Manager模块管理容器化应用的部署和生命周期

**组件扩展机制**：

- 所有组件实现统一的 `ComponentExecutor` 接口
- 通过 Activity 注册机制动态加载组件
- 支持自定义组件开发和插件化扩展
- 组件配置通过 JSON Schema 定义和验证
