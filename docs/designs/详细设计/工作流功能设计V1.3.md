...
- [工作流功能详细设计 V1.2](#工作流功能详细设计-v12)
  ...
  - [5. 实现要点与关键数据流](#5-实现要点与关键数据流)
    ...
    - [5.6 分布式调度实现](#56-分布式调度实现)
      - [5.6.1 多节点任务分配   - 将凭据添加到环境变量](#561-多节点任务分配-----将凭据添加到环境变量)
...
1. **前端层（User）**：YAML/JSON 编辑器实现工作流定义
...
1. **接收工作流定义**：
   - 从 API 请求获取 YAML/JSON 内容
   - 验证文件格式和编码

2. **语法解析**：
   - 使用 YAML/JSON 解析器解析内容
...
1. **Worker 启动**：
   ...
   // 注册 Activities
   worker.RegisterActivity(ShellActivity)
   worker.RegisterActivity(HttpActivity)
   worker.RegisterActivity(EmailActivity)
   // ...
...
4. **执行 Activity**：
   ...
   ```go
   func ShellActivity(ctx context.Context, input ComponentInput) (*ComponentOutput, error) {
       // 1. 解析参数
