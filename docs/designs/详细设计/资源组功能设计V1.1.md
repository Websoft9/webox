...
| **资源关联管理** ||||
| GET | `/api/v1/resource-groups/{id}/resources` | 获取资源组内的资源列表 | 支持按资源类型筛选，返回扁平化列表（不按类型分组） |
| PUT | `/api/v1/resources/resource-group` | 批量移动资源到指定资源组 | 请求体包含 `resource_codes` 数组和 `resource_group_id`（必传字段，值可为 null 表示移至默认组）；通过 code 前缀自动识别资源类型 |
| **资源基础统计** ||||
...
批量移动多个资源到指定的资源组或默认资源组。支持跨类型资源的批量移动，通过资源 code 前缀自动识别资源类型。

**方法/路径**

```
PUT /api/v1/resources/resource-group
```
...
| 字段名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| `resource_codes` | array[string] | 是 | 资源编码数组，格式：`{resource_type}_{identifier}`<br/>示例：`["server_123", "database_456", "secret_789"]` |
| `resource_group_id` | integer \| null | 是 | 目标资源组ID（必传字段）<br/>- **传递具体ID**：移至指定资源组<br/>- **传递 `null`**：移至项目默认资源组（自动从资源中获取 project_id） |
...
```json
{
  "resource_codes": ["server_123", "database_456", "secret_789"],
...
{
  "code": 200,
  "message": "Resources moved successfully",
  "data": null
}
```
...
1. **资源类型识别**：通过 code 前缀自动识别资源类型（从 `resource_types` 表查询 code 和 table_name 映射）
2. **权限校验**：
   - 如果指定了目标资源组ID，验证用户对该资源组的所有权
   - 验证资源组与资源属于同一项目
3. **默认资源组处理**：
...
**参数规则：**
- `project_id` 和 `resource_group_id` **互斥**，不能同时传递（Controller 层验证）
